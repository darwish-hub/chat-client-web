...
**Branch**: `001-chat-test-client` | **Date**: 2026-05-10 | **Spec**: [spec.md](spec.md)  
**Input**: Feature specification from `/specs/001-chat-test-client/spec.md`

**Note**: This template is filled in by the `/speckit.plan` command. See `.specify/templates/plan-template.md` for the execution workflow.

## Summary

Build a standalone web test client that exercises every feature of the ChatHub real-time chat service. The client is a browser-based single-page application (SPA) that connects via native WebSockets, renders a multi-user chat UI, and provides manual controls to test text messaging, live voice streaming, file/video uploads, presence indicators, typing indicators, message replies, reconnection recovery, and error handling.
...
**Testing**: Manual browser-based validation; automated test scenarios built into the client UI  
...
```text
client/
├── index.html                 # Single-page shell with all panels
├── css/
│   └── main.css               # Layout, panels, responsive rules, dark mode
├── js/
│   ├── main.js                # Bootstrap: auth, router, global event bus
...
│   ├── ui/
│   │   ├── authPanel.js       # JWT token input, connect button, connection status
│   │   ├── conversationList.js
│   │   ├── messageList.js     # Scrollable list with reply threading UI
│   │   ├── composer.js        # Text input, send, reply-to selector
│   │   ├── voiceRecorder.js   # Record/start/stop + live stream toggle
│   │   ├── fileUploader.js    # Drag-and-drop + file picker for attachments
│   │   ├── presenceBar.js     # Online users, typing dots
│   │   └── notifications.js   # Toast/error overlay for server errors
│   └── media/
...
**Structure Decision**: Single static HTML/CSS/JS project with a layered architecture. The `client/` directory contains all source code. Layers are strictly ordered: `protocol/` (lowest) → `transport/` → `api/` → `state/` → `ui/` (highest). No circular dependencies permitted.
...
- [ ] **C01** Create `index.html` with semantic panels: Auth, Conversations, Messages, Composer, Presence, Logs
- [ ] **C02** Create `css/main.css` with flex/grid layout, panel scroll regions, mobile-responsive breakpoints, dark/light mode toggle
- [ ] **C03** Create `js/config.js` with `API_BASE`, `WS_URL`, `MAX_TEXT_LENGTH`, `PING_INTERVAL_MS`, `RECONNECT_DELAY_MS`
- [ ] **C04** Create `js/protocol/messageTypes.js` — export string constants for every `type` value in the wire protocol
- [ ] **C05** Create `js/protocol/builders.js` — factory functions: `buildJoinService()`, `buildTextMessage()`, `buildVoiceChunk()`, `buildTyping()`, `buildAck()`, `buildPong()`
- [ ] **C06** Create `js/transport/wsClient.js` — `connect(token)`, `send(json)`, `sendBinary(data)`, `close()`, `onMessage(callback)`, `onClose(callback)`
- [ ] **C07** Create `js/transport/heartbeat.js` — on `ping`, reply with `pong`; if no ping for `PING_INTERVAL_MS * 2`, flag connection suspicious
- [ ] **C08** Create `js/transport/sendQueue.js` — serialize all sends through a queue so `ws.send()` is never called concurrently
- [ ] **C09** Create `js/ui/authPanel.js` — input for JWT, connect/disconnect buttons, connection status indicator (connected / disconnected / reconnecting)
- [ ] **C10** Wire `main.js` bootstrap: read token from `localStorage`, attempt auto-connect, render auth state

**Independent Test**:  
Open `index.html` in browser, paste a JWT, click Connect. The Logs panel shows `user_joined` after `join_service`. A `ping` arrives every 15s and client responds `pong`. Disconnecting shows clean close.
...
- [ ] **C11** Create `js/state/conversationStore.js` — `addOrUpdate(conversation)`, `get(id)`, `list()`, `setCurrent(id)`
- [ ] **C12** Create `js/state/messageStore.js` — `add(envelope)`, `getForConversation(conversationId)`, `ack(messageId)`, `findById(id)`
- [ ] **C13** Create `js/ui/conversationList.js` — render store entries, highlight current, click to switch
- [ ] **C14** Create `js/ui/messageList.js` — render messages chronologically, auto-scroll to bottom, show sender name, timestamp, delivery checkmark on `delivered`
- [ ] **C15** Create `js/ui/composer.js` — textarea with `MAX_TEXT_LENGTH` counter, Send button emits `text_message` envelope with client-generated UUID
- [ ] **C16** Handle `message_received` in `wsClient.js` — route to `messageStore.add()`, re-render message list
- [ ] **C17** Handle `delivered` in `wsClient.js` — update message store, render delivery checkmark
- [ ] **C18** Create `js/api/conversations.js` — `createConversation(serviceId, title, participantIds)` and `listMyConversations()`
- [ ] **C19** Create `js/api/history.js` — `fetchHistory(conversationId, before, limit)`; call on conversation switch to backfill messages
- [ ] **C20** Add "New Conversation" button in UI to test creation flow end-to-end
...
- [ ] **C21** Create `js/media/audioCapture.js` — `requestMic()`, `startRecording(onChunk)`, `stopRecording()` using `MediaRecorder` with `timeslice: 200` (200ms chunks)
- [ ] **C22** Create `js/state/voiceSessionStore.js` — track active outbound `messageId`, `sequenceNumber`, `conversationId`; track inbound sessions by `(messageId, fromUserId)` with chunk buffers
- [ ] **C23** Update `js/protocol/builders.js` — `buildVoiceChunkStart(messageId, conversationId, sequenceNumber, isFinal)`
- [ ] **C24** Update `js/transport/wsClient.js` — send binary frames: first a text `voice_chunk` envelope, then immediately a binary `ArrayBuffer` with the audio payload
- [ ] **C25** Create `js/media/audioPlayer.js` — `enqueueChunk(messageId, sequenceNumber, arrayBuffer)`, sort by sequence, decode via `AudioContext`, schedule playback with `AudioBufferSourceNode` to eliminate gaps
- [ ] **C26** Handle inbound `voice_chunk` (text envelope + binary frame) in parser: route to `audioPlayer.enqueueChunk()` if live, or store if not yet playing
- [ ] **C27** Update `js/ui/voiceRecorder.js` — large "Push to Talk" button (hold to record, release to stop), visual recording indicator, live stream toggle checkbox, display elapsed seconds
- [ ] **C28** On `isFinal: true`, show a playable `<audio>` element in the message list pointing to the assembled message (or placeholder until server assembles)
- [ ] **C29** Add UI toggle: "Simulate packet loss" — randomly drop 1 in 20 outbound chunks to test server/client gap recovery (visual warning, not real protocol change)
...
- [ ] **C30** Create `js/api/upload.js` — `uploadFile(file, onProgress)` using `FormData` + `XMLHttpRequest` (for progress) or `fetch` with `ReadableStream` tracking; returns `{ blobId, url }`
- [ ] **C31** Update `js/protocol/builders.js` — `buildFileAttachment(messageId, conversationId, blobId, fileName, mimeType, sizeBytes, durationMs?)`
- [ ] **C32** Create `js/ui/fileUploader.js` — drag-and-drop zone + `<input type="file">`, file type icon preview, upload progress bar, cancel button
- [ ] **C33** Update `js/ui/composer.js` — attach button opens file picker; on upload completion, auto-sends `file_attachment` message
- [ ] **C34** Handle `message_received` with `type: "voice" | "video" | "file"` in message list: render media card with file name, size, MIME icon, download link
- [ ] **C35** Create `js/api/download.js` — `download(blobId)` opens `GET /api/download/{blobId}` in new tab or uses `fetch` + `URL.createObjectURL` for in-app download
- [ ] **C36** Create `js/media/videoPreview.js` — for `type: "video"`, render `<video controls src="...">` using the download URL (or pre-signed URL if available)
- [ ] **C37** Add "Upload from URL" test helper (paste a public URL, client fetches it and uploads) — useful for CI/automated testing
...
- [ ] **C38** Create `js/state/presenceStore.js` — `setOnline(serviceId, user)`, `setOffline(serviceId, userId)`, `getOnline(serviceId)`, `setTyping(conversationId, userId, isTyping)`
- [ ] **C39** Create `js/ui/presenceBar.js` — render avatars/names of online users in current service; show green dot for online, grey for offline
- [ ] **C40** Handle `user_joined` / `user_left` in `wsClient.js` — update presence store, re-render presence bar
- [ ] **C41** Create `js/ui/typingIndicator.js` — when `typing` event received with `isTyping: true`, show "Alice is typing..." dots above composer; clear after 5s of no refresh or on `isTyping: false`
- [ ] **C42** Add debounced typing emit in `js/ui/composer.js` — on keydown, emit `typing: true`; 3s after last keystroke, emit `typing: false`
- [ ] **C43** Create `js/api/presence.js` — `fetchOnlineUsers(serviceId)` for initial load when joining a service
- [ ] **C44** Add "Simulate disconnect" button — forces `ws.close()` without sending `leave_service` to test server-side timeout detection
...
- [ ] **C45** Update `js/ui/messageList.js` — add "Reply" action button on hover/focus of each message bubble
- [ ] **C46** Update `js/ui/composer.js` — when replying, show a compact preview of the original message above the textarea with a dismiss button; include `replyToId` in outbound envelope
- [ ] **C47** Update `js/protocol/builders.js` — all builders accept optional `replyToId`
- [ ] **C48** Update `js/ui/messageList.js` — render reply messages with a visual thread line connecting to the original; clicking the reply scrolls to and highlights the original message
- [ ] **C49** Create `js/api/history.js` — `fetchThread(conversationId, messageId)` calling `GET /api/conversations/{conversationId}/messages/{messageId}/replies`; add "View Thread" button on messages with replies
- [ ] **C50** Add reply support for voice and file messages — reply to a voice memo with a text message, reply to a file with another file, etc.
...
- [ ] **C51** Update `js/transport/wsClient.js` — automatic reconnect with exponential backoff (`1s, 2s, 4s, 8s, max 30s`); on reconnect, re-send `join_service` for the current service
- [ ] **C52** On reconnect, call `fetchHistory(conversationId, lastKnownMessageTimestamp)` for every open conversation to backfill missed messages
- [ ] **C53** Handle `error` messages from server — display toast with `code` and `message`; specific handling for `rate_limit_exceeded` (disable send button for 5s), `not_participant` (redirect to conversation list), `invalid_message` (highlight composer in red)
- [ ] **C54** Add duplicate detection in `js/state/messageStore.js` — ignore `message_received` envelopes with already-known `id`
- [ ] **C55** Add local message ordering guard — if a message arrives with `createdAt` older than the last visible message, insert it in the correct chronological position rather than appending
- [ ] **C56** Add network throttle simulation — dropdown to select "Fast 4G", "Slow 3G", "Offline" using Chrome DevTools Protocol or manual delay injection for testing
- [ ] **C57** Add "Send burst" test button — sends 20 text messages rapidly to test rate limiting UI feedback (should show toasts and temporarily disable input)
- [ ] **C58** Add "Multi-device" simulation — allow opening a second WebSocket connection in the same tab (or instructions to open a second tab) with a different user token to test multi-device presence
...
- [ ] **C59** Add "Protocol Log" panel — raw JSON of every sent/received frame with timestamp, direction arrow, expandable pretty-print
- [ ] **C60** Add "Metrics Dashboard" mini-panel — connection uptime, messages sent/received, average latency (time between send and `delivered`), voice chunk latency histogram, current send queue depth
- [ ] **C61** Add "Test Scenarios" section with one-click buttons:  
...
- [ ] **C62** Add export button — download protocol log as `.ndjson` for post-mortem analysis
- [ ] **C63** Add `README.md` with: prerequisites, `npx serve` instructions, how to generate a test JWT, screenshot of each panel, description of every test scenario
- [ ] **C64** Validate all manual tests against a running local server (`docker-compose up`)
...
1. `index.html` + `css/main.css` — static shell with all panels
2. `js/config.js` — environment constants
3. `js/protocol/messageTypes.js` + `js/protocol/builders.js` + `js/protocol/parsers.js`
4. `js/transport/sendQueue.js` + `js/transport/heartbeat.js` + `js/transport/wsClient.js`
5. `js/state/conversationStore.js` + `js/state/messageStore.js`
6. `js/ui/authPanel.js` + `js/ui/conversationList.js` + `js/ui/messageList.js` + `js/ui/composer.js`
7. `js/api/conversations.js` + `js/api/history.js`
8. `js/media/audioCapture.js` + `js/media/audioPlayer.js` + `js/state/voiceSessionStore.js` + `js/ui/voiceRecorder.js`
9. `js/api/upload.js` + `js/api/download.js` + `js/ui/fileUploader.js` + `js/media/videoPreview.js`
10. `js/state/presenceStore.js` + `js/ui/presenceBar.js` + `js/ui/typingIndicator.js` + `js/api/presence.js`
11. `js/ui/messageList.js` (reply rendering) + `js/api/history.js` (thread endpoint)
12. `js/transport/wsClient.js` (reconnection) + `js/state/messageStore.js` (duplicate detection, ordering)
