...
- Any static file server (e.g., `npx serve`, Python `http.server`, VS Code Live Server)

---

## Step 1: Serve the Client

The client is a React single-page application. A build step is required.

### Option A: Using the React Development Server
...
### Option B: Serve the Production Build
...
Edit `js/config.js` to change defaults:
...
Rebuild after modifying source code; the development server hot-reloads on changes.
